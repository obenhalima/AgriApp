...
export function TreatmentCalendar({requests,plantings,children,selection}:{requests:any[];plantings:any[];children:(rows:any[])=>ReactNode;selection?:{status:string;stock:string;sequence:number}|null}){
 ...
 const select=(key:keyof typeof blank,label:string,options:{id:string;name:string}[])=> <label className="text-caption" key={key}>{label}<select aria-label={label} className="block w-full rounded border border-border bg-surface-raised p-2 mt-1" value={filters[key]} onChange={e=>change(key,e.target.value)}><option value="">Tous</option>{options.map(o=><option key={o.id} value={o.id}>{o.name}</option>)}</select></label>
 ...
 const filterPanel = <Card><h2 className="font-bold mb-3">Filtrer et trier les traitements</h2><div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3">
   {select('farm','Ferme',farms)}
   {select('greenhouse','Serre',places.filter(p=>!filters.farm||p.farm_id===filters.farm))}
   {select('target','Cible',names(requests.flatMap(r=>[r.target_name,...treatmentLines(r).map((p:any)=>p.target_name)])).map(name=>({id:name,name})))}
   ...
   {select('status','Statut',[...Object.entries(labels).map(([id,name])=>({id,name})),{id:'actifs',name:'À valider ou à confirmer'}])}
 ...
 return <div className="space-y-4">
  <div className="grid grid-cols-1 lg:grid-cols-[280px_1fr] gap-4">
   <Card padding="none" className="overflow-hidden"><div className="p-3 border-b border-border"><h2 className="flex items-center gap-2 font-bold"><Calendar size={14} className="text-info"/>Calendrier des traitements</h2><div className="flex items-center justify-between mt-2"><Button variant="ghost" size="icon-sm" aria-label="Mois précédent" onClick={()=>shift(-1)}><ChevronLeft size={14}/></Button><span className="text-caption capitalize">{new Date(`${month}-01T12:00:00Z`).toLocaleDateString('fr-FR',{month:'long',year:'numeric',timeZone:'UTC'})}</span><Button variant="ghost" size="icon-sm" aria-label="Mois suivant" onClick={()=>shift(1)}><ChevronRight size={14}/></Button></div></div>
