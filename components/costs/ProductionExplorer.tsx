...
const button = 'rounded-lg border border-border bg-surface-raised px-4 py-3 text-sm text-fg-primary shadow-sm transition-colors hover:bg-surface-hover hover:border-brand focus-visible:outline focus-visible:outline-2 focus-visible:outline-brand'
export function ProductionExplorer({data,farm,variety,client,onFarm,partialPeriod=false}:{data:PerformanceData;farm:string;variety:string;client:string;onFarm:(id:string)=>void;partialPeriod?:boolean}) {
 ...
 return <section aria-label="Exploration production et coût par kg" className="space-y-5 min-w-0">
  ...
  {view.incomplete&&<p role="status" className="rounded-lg border border-warning bg-warning-dim p-4 text-sm leading-relaxed text-fg-secondary">Coûts incomplets ou provisoires. Les contrôles de rapprochement globaux restent applicables ; un faible coût/kg ne prouve pas une meilleure performance.</p>}
  ...
  {!greenhouse&&<div className="grid gap-3 md:grid-cols-2">{(farm?view.rows:farms).map(r=><button key={r.id} className={`${button} text-left`} onClick={()=>{setCategory(null);setCostId('');if(farm)setGreenhouse(r.id);else onFarm(r.id)}}><strong className="block">{r.name} →</strong><span className="mt-2 block text-xs tabular-nums text-fg-tertiary">{fmt(r.kg)} kg · {fmt(r.direct+r.shared)} DH · {fmt(r.kg>0&&!r.costMissing?(r.direct+r.shared)/r.kg:null)} DH/kg</span></button>)}</div>}
