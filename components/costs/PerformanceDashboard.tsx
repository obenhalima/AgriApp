...
const tooltip = { background: 'var(--bg-card)', border: '1px solid var(--border)', borderRadius: 8, color: 'var(--tx-1)', fontSize: 12 }
const axis = { fontSize: 10, fill: 'var(--tx-3)' }

export function PerformanceDashboard({ data, rows, ranked, basis, metric, metricLabel, partialPeriod, onSelect, exploration = false }: {
  data: PerformanceData; rows: PerformanceRow[]; ranked: PerformanceRow[]; basis: RevenueBasis; metric: PerformanceMetric; metricLabel: string; partialPeriod: boolean; onSelect: (id: string) => void; exploration?: boolean
}) {
  ...
  const comparison = (exploration ? rows.filter(r=>performanceValue(r,metric,basis)!=null) : ranked).slice(0, 8).map(r => ({ id: r.id, name: r.name, value: performanceValue(r, metric, basis)! }))
  const kpis = [
  ...
  return <div className="space-y-4">
    <div aria-label="Synthèse du périmètre filtré" className={`grid grid-cols-1 gap-3 sm:grid-cols-2 ${kpis.length===4?'2xl:grid-cols-4':'xl:grid-cols-3'}`}>{kpis.map(k => <KPICard key={k.label} {...k} variant="compact" />)}</div>
    <div className="grid gap-4 xl:grid-cols-2">
      <section className="card min-w-0 p-4" aria-label="Progression des récoltes">
        <div className="flex flex-wrap items-start justify-between gap-3"><div><h2 className="font-semibold">Progression des récoltes</h2><p className="text-xs" style={{ color: 'var(--tx-3)' }}>Volumes {cumulative ? 'cumulés depuis le début du périmètre' : 'par période'} · kg</p></div>
          <div className="flex flex-wrap gap-2"><Select aria-label="Granularité des courbes" style={{ width: 'auto' }} value={grain} onChange={e => setGrain(e.target.value as typeof grain)}><option value="month">Mensuelle</option><option value="week">Hebdomadaire</option></Select><button className="btn btn-secondary" aria-pressed={cumulative} onClick={() => setCumulative(v => !v)}>{cumulative ? 'Cumulé' : 'Par période'}</button></div>
        </div>
        {timeline.length ? <><div className="mt-4" style={{ height: 260 }}><ResponsiveContainer width="100%" height="100%"><AreaChart data={timeline} margin={{ top: 10, right: 12, left: 0, bottom: 5 }}><defs><linearGradient id="performanceHarvestFill" x1="0" y1="0" x2="0" y2="1"><stop offset="0%" stopColor="#06b6d4" stopOpacity={.3}/><stop offset="100%" stopColor="#06b6d4" stopOpacity={.02}/></linearGradient></defs><CartesianGrid vertical={false} stroke="var(--border)" strokeDasharray="3 3"/><XAxis dataKey="label" tick={axis} axisLine={false} tickLine={false}/><YAxis tick={axis} tickFormatter={ticks} axisLine={false} tickLine={false}/><Tooltip contentStyle={tooltip} formatter={(v: number) => [`${fmt(v)} kg`, cumulative ? 'Récolte cumulée' : 'Récolte de la période']}/><Area type="linear" dataKey={cumulative ? 'cumulative' : 'kg'} stroke="#06b6d4" strokeWidth={3} fill="url(#performanceHarvestFill)" dot={{ r: 3 }} isAnimationActive={false}/></AreaChart></ResponsiveContainer></div><details className="mt-3 rounded-lg border border-border p-3 text-xs text-fg-secondary"><summary>Données de la courbe</summary><div className="max-h-40 overflow-auto"><table className="tbl w-full tabular-nums"><tbody>{timeline.map(p => <tr key={p.date}><th className="text-left">{p.label}</th><td>{fmt(p.kg)} kg / période</td><td>{fmt(p.cumulative)} kg cumulés</td></tr>)}</tbody></table></div></details></> : <p className="py-20 text-center text-sm">Aucune récolte dans le périmètre filtré.</p>}
      </section>
      <section className="card min-w-0 p-4" aria-label="Classement graphique"><h2 className="font-semibold">{exploration ? 'Explorer le périmètre' : 'Comparaison des performances'}</h2><p className="text-xs" style={{ color: 'var(--tx-3)' }}>{metricLabel} · {exploration ? '8 premiers éléments du périmètre, sans classement · cliquez pour explorer' : '8 premiers éligibles · cliquez sur une barre pour le détail'}</p>
        {comparison.length ? <div style={{ height: Math.max(260, comparison.length * 40) }}><ResponsiveContainer width="100%" height="100%"><BarChart data={comparison} layout="vertical" margin={{ top: 20, right: 22, left: 0, bottom: 5 }}><CartesianGrid horizontal={false} stroke="var(--border)"/><XAxis type="number" tick={axis} tickFormatter={ticks}/><YAxis type="category" dataKey="name" width={120} tick={{ ...axis, fontSize: 10 }} tickFormatter={(s: string) => s.length > 22 ? s.slice(0, 21) + '…' : s}/><Tooltip contentStyle={tooltip} formatter={(v: number) => [fmt(v), metricLabel]}/><Bar dataKey="value" radius={[0, 4, 4, 0]} maxBarSize={24} onClick={entry => onSelect(entry.id)} cursor="pointer" isAnimationActive={false}>{comparison.map((r, i) => <Cell key={r.id} fill={colors[i % colors.length]}/>)}</Bar></BarChart></ResponsiveContainer></div> : <p className="py-16 text-center text-sm">Aucun classement fiable pour ce critère. Vérifiez les coûts, les récoltes ou incluez les cycles en cours.</p>}
        {!exploration && ranked.length > 0 && <button className="mt-2 text-left text-sm" onClick={() => onSelect(ranked[0].id)} style={{ color: 'var(--neon)' }}>Meilleur résultat observé — provisoire : <strong>{ranked[0].name}</strong> · {fmt(performanceValue(ranked[0], metric, basis))}</button>}
      </section>
    </div>
    {!exploration&&<section className="card p-4"><h2 className="font-semibold">Où vont les charges ?</h2><p className="mb-4 text-xs" style={{ color: 'var(--tx-3)' }}>Répartition des charges réelles imputées au périmètre filtré · DH</p><div className="grid gap-x-8 gap-y-3 md:grid-cols-2">{categories.map((c, i) => <div key={c.name}><div className="mb-1 flex justify-between gap-3 text-xs"><span>{c.name}</span><strong>{fmt(c.value)} DH</strong></div><div style={{ height: 6, background: 'var(--border)', borderRadius: 4 }}><div style={{ height: '100%', width: `${Math.min(100, Math.abs(c.value) / Math.max(1, ...categories.map(x => Math.abs(x.value))) * 100)}%`, background: colors[i % colors.length], borderRadius: 4 }}/></div></div>)}</div>{!categories.length && <p className="text-sm">Aucune charge imputée ; un coût nul ne signifie pas une production gratuite.</p>}</section>}
  </div>
}
