...
export function AppShell({ children }: { children: React.ReactNode }) {
  ...
  if (checking) {
    return (
      <div style={{
        ...
        <div style={{ fontFamily:'var(--font-mono)', fontSize:11, color:'var(--tx-3)', letterSpacing:2 }}>
          INITIALISATION...
        </div>
      </div>
    )
  ...
  return (
    <div style={{ display:'flex', minHeight:'100vh', background:'var(--bg-base)', transition:'background .4s ease' }}>
      <Sidebar />
      <div style={{ flex:1, display:'flex', flexDirection:'column', marginLeft:'var(--sidebar-w)', minHeight:'100vh' }}>
        <Topbar />
        <main style={{ flex:1, padding:'22px 24px', position:'relative', zIndex:1, background:'var(--bg-base)', transition:'background .4s ease' }}>
          {children}
        </main>
      </div>
    </div>
