...
export type AIChatInput = {
  ...
  messages: ChatMessage[]     // historique + nouveau message en dernier
}

export async function sendChatMessage(input: AIChatInput): Promise<{ reply: string; model: string }> {
  ...
  if (!parsed.reply) throw new Error('Réponse vide de l\'IA')
  return { reply: parsed.reply, model: parsed.model ?? 'gemini' }
}
...
export function buildChatContext(params: {
  ...
  const topLines = leaves
    .map(l => {
      ...
      return { label: l.category_label, type: l.category_type, budget: l.budget, actual: l.actual, variancePct }
    })
