-- ============================================================
-- Migration 069 — Le bordereau station écrit le CA réalisé sur les lots
--
-- BUG : après validation d'un bordereau station, l'onglet « CA Réalisé »
-- restait vide. La RPC admin_validate_station_settlement (041) allouait bien
-- (qty_priced_kg, tri_status='priced', station_settlement_allocations) mais
-- n'écrivait JAMAIS harvest_lots.ca_amount ni harvest_lots.price_per_kg —
-- or l'onglet CA Réalisé lit harvest_lots.ca_amount.
--
-- FIX : on recrée les deux RPC :
--   • validate   → accumule ca_amount += qty_allouée × prix_ligne
--                  et pose price_per_kg = prix_ligne sur le lot.
--   • unvalidate → décrémente ca_amount du montant alloué (via le prix
--                  stocké dans l'allocation) et remet price_per_kg à NULL
--                  quand le lot n'est plus tarifé du tout.
--
-- Colonnes : harvest_lots.ca_amount + price_per_kg (024),
--            station_settlement_allocations.price_per_kg (040).
-- Sécurité : SECURITY DEFINER + is_admin(auth.uid()), inchangée.
-- ============================================================

SET search_path = public;

-- ─── VALIDATE (recréée avec écriture du CA) ──────────────────────────
...
BEGIN
  ...
  LOOP
    ...
    LOOP
      ...
      -- Update du lot : qty + CA réalisé + prix + statut
      UPDATE harvest_lots
         SET qty_priced_kg = COALESCE(qty_priced_kg, 0) + v_to_allocate,
             ca_amount     = COALESCE(ca_amount, 0) + v_to_allocate * COALESCE(v_line.price_per_kg, 0),
...
'Valide un bordereau station : alloue FIFO chaque ligne sur les dispatches triés, accumule ca_amount + price_per_kg sur les lots, bascule tri_status=priced quand un lot est totalement payé.';

-- ─── UNVALIDATE (recréée : décrémente aussi le CA) ───────────────────
...
BEGIN
  ...
  -- Décrémente qty_priced_kg + ca_amount sur chaque lot, retire priced si besoin
  FOR v_alloc IN
    SELECT a.id, a.harvest_lot_id, a.qty_allocated_kg, a.price_per_kg
  ...
  LOOP
    UPDATE harvest_lots
       SET qty_priced_kg = GREATEST(0, COALESCE(qty_priced_kg, 0) - v_alloc.qty_allocated_kg),
           ca_amount     = GREATEST(0, COALESCE(ca_amount, 0) - v_alloc.qty_allocated_kg * COALESCE(v_alloc.price_per_kg, 0)),
           price_per_kg  = CASE
             WHEN (COALESCE(qty_priced_kg, 0) - v_alloc.qty_allocated_kg) <= 0 THEN NULL
             ELSE price_per_kg
           END,
           tri_status = CASE
...
'Annule la validation d''un bordereau : supprime allocations, décrémente qty_priced_kg + ca_amount, remet price_per_kg à NULL si le lot n''est plus tarifé, repasse status=brouillon.';

-- ─── Rattrapage : recalcule ca_amount pour les bordereaux DÉJÀ validés ──
-- (les lots tarifés avant ce correctif ont ca_amount NULL alors que des
--  allocations existent → on reconstruit ca_amount depuis les allocations.)
UPDATE harvest_lots hl
   SET ca_amount = agg.ca,
       price_per_kg = COALESCE(hl.price_per_kg, agg.last_price)
  FROM (
    SELECT a.harvest_lot_id,
           SUM(a.qty_allocated_kg * a.price_per_kg) AS ca,
           MAX(a.price_per_kg)                      AS last_price
      FROM station_settlement_allocations a
     GROUP BY a.harvest_lot_id
  ) agg
 WHERE hl.id = agg.harvest_lot_id
   AND COALESCE(hl.ca_amount, 0) = 0;
