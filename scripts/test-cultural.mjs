...
try{
 ...
 await page.getByRole('button',{name:'Ouvrir Programme QA'}).first().click();await page.getByRole('button',{name:'Approuver niveau 1'}).click()
 ...
 await expect(page.getByLabel('Date réelle',{exact:true})).toHaveAttribute('aria-invalid','true')
 await expect(page.getByLabel('Date réelle',{exact:true})).toBeFocused()
 await expect(page.getByLabel('Observation',{exact:true})).toHaveAttribute('aria-invalid','true')
 await page.getByRole('button',{name:'Observation du réalisé : saisissez au moins 5 caractères.',exact:true}).click()
 await expect(page.getByLabel('Observation',{exact:true})).toBeFocused()
 ...
 await page.getByRole('button',{name:'Confirmer les consommations'}).click()
 await expect(page.getByRole('alert').filter({hasText:'hors solution restant en bac'}).last()).toBeVisible()
 if(confirmations!==0)throw Error('Unacknowledged consumption sent')
 await page.getByRole('checkbox',{name:/Je confirme les quantités réellement consommées/}).check()
 await page.getByRole('button',{name:'Confirmer les consommations'}).click()
 await expect(page.getByRole('status').filter({hasText:'Application confirmée'})).toBeVisible()
 await expect(page.getByText(/Réalisée/).first()).toBeVisible();if(confirmations!==1)throw Error('Confirmation count')
 await page.locator('.modal-close').click()
 await page.getByRole('button',{name:'Voir les interventions : À réaliser',exact:true}).click()
 await expect(page.getByLabel('Filtrer statut')).toHaveValue('approuvee')
 await expect(page.getByRole('columnheader',{name:'Serres',exact:true})).toBeVisible()
 await page.getByRole('button',{name:'Réinitialiser les filtres'}).click()
 await page.screenshot({path:'outputs/cultural-phyto-theme.png',fullPage:true})
 ...
 await expect(page.getByRole('heading',{name:'Aucune demande en attente'})).toBeVisible()
 await expect.poll(()=>page.evaluate(()=>document.documentElement.scrollWidth<=innerWidth+2)).toBe(true)
