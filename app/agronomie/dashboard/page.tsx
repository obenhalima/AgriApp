...
import { Activity, Leaf, RefreshCw, ArrowUpRight, Wheat, Gauge, Target, Wallet, Scale, Droplets, Package } from 'lucide-react'
...
const panel=`${styles.panel} min-w-0 border border-border bg-surface-raised p-5`
...
function Dashboard({entry}:{entry:AnalyticalScope}){
 ...
 return <div className={`${styles.dashboard} min-w-0 space-y-6`}>
  <PageHeader title="Mon exploitation" subtitle="Pilotage · Vue 360" icon={Leaf} iconColor="#10b981" description="Vos résultats, vos priorités et les décisions du jour. Une vue claire de votre exploitation." actions={<button className={`${input} flex items-center gap-2`} disabled={busy||!campaign} onClick={()=>setReload(x=>x+1)}><RefreshCw size={16}/>Actualiser</button>}/>
  ...
  {!campaign?<div className={panel}>Choisissez une campagne pour ouvrir la vue 360. Aucun chiffre n’est présumé avant le chargement.</div>:busy?<div role="status" className={panel}>Chargement des indicateurs de la campagne…</div>:data&&production&&activities&&<>
   ...
   <h2 className={styles.sectionLabel}>Les chiffres essentiels</h2>
   <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 xl:grid-cols-4">{kpis.map(([label,value,unit],index)=>{const href=index===3&&canCost?analyticalHref('/couts/pilotage',domain!,{campaign,farm}):index===4&&canCost?analyticalHref('/couts/performance',domain!,{campaign,farm,metric:'costKg',level:'greenhouse'}):index===7?'#stock-details':index===6?'#water-details':index<3?'#production-details':index===5?'#agenda-details':null;const Icon=[Wheat,Gauge,Target,Wallet,Scale,Activity,Droplets,Package][index];const accent=['#168579','#168579','#16705b','#2966a0','#2966a0','#b77916','#168579','#2966a0'][index];const body=<><div className="mb-4 flex items-center justify-between gap-2"><p className="text-sm font-medium text-fg-secondary">{label}</p><span className={styles.kpiIcon}><Icon size={18}/></span></div><p className={styles.kpiValue}>{fmt(value as number|null|undefined??null)}</p><p className="mt-1 text-xs text-fg-tertiary">{unit}</p>{href&&<span className="mt-4 flex items-center justify-between border-t border-border pt-3 text-xs text-brand">Voir le détail <ArrowUpRight size={14}/></span>}</>;const style={'--accent':accent} as CSSProperties;return href?<Link key={String(label)} href={href} style={style} className={styles.kpi}>{body}</Link>:<div key={String(label)} style={style} className={styles.kpi}>{body}</div>})}</div>
   ...
   <h2 id="production-details" className={`${styles.sectionLabel} scroll-mt-24`}>Comprendre ma production</h2>
   <div className="grid gap-5 xl:grid-cols-2">
    <section className={panel}><h2 className="font-semibold">Production enregistrée par mois</h2><p className="mb-3 text-xs text-fg-tertiary">kg bruts · mois comportant des saisies</p>{readyHarvest&&production.timeline.length?<div className="h-64"><ResponsiveContainer width="100%" height="100%"><AreaChart data={production.timeline}><CartesianGrid vertical={false} stroke="#e2ebe5" strokeDasharray="3 5"/><XAxis dataKey="month" axisLine={false} tickLine={false} tick={{fill:"#6b8278",fontSize:11}}/><YAxis width={60}/><Tooltip formatter={(v:number)=>[fmt(v)+' kg','Récolté']}/><Area dataKey="quantity" stroke="#10b981" fill="#10b981" fillOpacity={0.15}/></AreaChart></ResponsiveContainer></div>:<p className="p-6 text-sm text-fg-secondary">{readyHarvest?'Aucune récolte enregistrée.':'Données de récolte indisponibles.'}</p>}</section>
    <section className={panel}><h2 className="font-semibold">Rendement enregistré par serre</h2><p className="mb-3 text-xs text-fg-tertiary">Top 8 · kg/m² planté · cycles potentiellement à des stades différents</p>{readyHarvest&&production.comparison.length?<div className="h-64"><ResponsiveContainer width="100%" height="100%"><BarChart data={production.comparison.slice(0,8)}><CartesianGrid vertical={false} stroke="#e2ebe5" strokeDasharray="3 5"/><XAxis dataKey="name" axisLine={false} tickLine={false} tick={{fill:"#6b8278",fontSize:11}}/><YAxis/><Tooltip formatter={(v:number)=>[fmt(v)+' kg/m²','Rendement']}/><Bar dataKey="yield" fill="#168579" radius={[5,5,0,0]}/></BarChart></ResponsiveContainer></div>:<p className="p-6 text-sm text-fg-secondary">Aucune comparaison disponible.</p>}</section>
   </div>
   ...
   <h2 className={styles.sectionLabel}>Ressources et suivi terrain</h2>
   <div className="grid gap-5 xl:grid-cols-2">
