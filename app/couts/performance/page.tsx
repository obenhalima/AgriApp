...
function PerformanceReport({entry}:{entry:AnalyticalScope}){
 ...
 const [metric,setMetric]=useState<PerformanceMetric>(Object.keys(metricLabels).includes(entry.metric)?entry.metric as PerformanceMetric:'yield'),[basis,setBasis]=useState<RevenueBasis>('estimate'),[includeOpen,setIncludeOpen]=useState(true)
 ...
 return <section className="performance-report space-y-4" style={{color:'var(--tx-1)'}}>
  ...
  {mode==='compare'&&<div className="card flex flex-wrap items-end gap-3 p-4">
   ...
   <p className="w-full rounded-lg border border-border bg-surface-input p-3 text-sm text-fg-secondary" role="status">{includeOpen?'Comparaison à date — résultats provisoires. Les cultures en cours sont incluses : charges enregistrées divisées par les kilos déjà récoltés sur la période sélectionnée. Les résultats évolueront avec les prochaines saisies.':'Comparaison limitée aux cycles terminés. Cochez l’option ci-dessus pour comparer aussi les cultures en cours.'}</p>
  </div>}
  <details className="rounded-lg border border-border bg-surface-raised p-4 text-xs leading-relaxed text-fg-tertiary"><summary className="cursor-pointer">Méthode de calcul et limites de comparaison</summary><p className="mt-2">Surface = cumul des surfaces plantées des cycles retenus, pas la surface cadastrale. Kg = toutes les catégories récoltées, déchets inclus. {start||end?'Les coûts et récoltes sont limités à la période : ce ne sont pas des marges de cycle complet.':'Comparaison des cycles complets et en cours selon le filtre ci-dessus.'} Les cycles en cours sont inclus par défaut pour une comparaison à date ; décochez l’option pour limiter le classement aux cycles terminés. Les ratios sont pondérés par les quantités ou surfaces, jamais moyennés entre eux.</p>
  {data?.revenue_available&&<p className="text-sm text-fg-tertiary">CA estimé = catégories récoltées × prix saisis (export/local). CA station = montants renseignés sur les lots, datés par leur récolte ; ce n’est ni le CA facturé ni les encaissements. Montants interprétés en DH selon les données saisies ; vérifier les devises et les tarifs avant toute décision. Une tarification station absente ou partielle empêche le calcul de sa marge.</p>}
  </details>
  ...
  {busy&&<p role="status" className="card p-6 text-center text-sm text-fg-tertiary">Chargement des performances…</p>}{error&&<p role="alert" className="rounded-lg border border-danger bg-danger-dim p-4 text-sm text-danger">{error}</p>}
  ...
  {result&&mode==='compare'&&<>
   ...
   <div className="card overflow-hidden"><div className="flex items-center justify-between p-4"><h2 className="font-semibold">Comparatif détaillé</h2><span className="text-xs" style={{color:'var(--tx-3)'}}>{rows.length} ligne(s) · {ranked.length} classée(s)</span></div><div className="overflow-x-auto"><table className="tbl w-full whitespace-nowrap text-right text-xs tabular-nums"><thead style={{background:'var(--bg-card2)',color:'var(--tx-3)'}}><tr>{['Rang','Périmètre','Surface m²','Récolté kg','kg/m²','Coûts réels DH','DH/kg',basis==='estimate'?'CA estimé DH':'CA station saisi DH','Marge DH','Marge DH/kg','Marge DH/m²','Budget saisi DH','Budget/kg cible','Qualité','Détail'].map(h=><th key={h} className="p-3">{h}</th>)}</tr></thead><tbody>{rows.map(r=>{
    ...
    return <tr key={r.id} className="border-t border-border hover:bg-surface-hover"><td className="p-3">{rank<0?'—':rank+1}</td><th className="p-3 text-left">{r.name}</th><td className="p-3">{fmt(r.area)}</td><td className="p-3">{fmt(r.kg)}</td><td className="p-3">{fmt(performanceValue(r,'yield',basis))}</td><td className="p-3">{fmt(r.direct+r.shared)}</td><td className="p-3" title={String(performanceValue(r,'costKg',basis)??'')}>{fmt(performanceValue(r,'costKg',basis))}</td>
     ...
     <td className="p-3 text-left text-xs">{[r.open?'Cycle en cours / non terminé':'Cycle terminé',r.costMissing?'Coûts absents sur une plantation':null,r.provisional?'Valorisation provisoire':null,basis==='station'&&r.stationMissing?'Station non tarifée / incomplète':null,basis==='estimate'&&r.estimateMissing?'Récolte ou prix manquant':null].filter(Boolean).join(' · ')}</td><td className="p-3"><button className="btn btn-secondary" onClick={()=>setSelected(r.id===selected?'':r.id)}>Voir le détail</button></td></tr>
   })}</tbody></table>{!rows.length&&<p className="p-4">Aucune plantation dans ce périmètre.</p>}</div></div>
   <p className="text-sm text-fg-tertiary">Budget/kg cible : budget saisi ÷ objectif de production du cycle ; indisponible pour une période partielle. Coût réel/kg : charges imputées ÷ kg récoltés. Les valeurs inférieures à 0,005 DH sont signalées plutôt qu’arrondies à zéro.</p>
   {detail&&data&&<section ref={detailRef} tabIndex={-1} aria-label="Détail de performance" className="performance-detail card scroll-mt-24 space-y-4 p-5">
 ...
 <style jsx>{`.performance-report :global(label){font-size:11px;color:var(--tx-3);min-width:0} .performance-report :global(label select),.performance-report :global(label input[type=date]){display:block;margin-top:6px} .performance-report :global(td),.performance-report :global(th){border-color:var(--border)} .performance-detail>h3{padding-top:16px;border-top:1px solid var(--border);font-size:12px;color:var(--tx-2)} .performance-detail>p{font-size:13px;line-height:1.7;padding:10px 12px;border-radius:8px;background:var(--bg-card2)}`}</style></section>
}
